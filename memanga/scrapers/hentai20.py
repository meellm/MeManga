"""
Hentai20 scraper
Website: https://hentai20.io
Uses WordPress with MangaReader theme
Adult content - 18+
"""
...
class Hentai20Scraper(BaseScraper):
    """Scraper for hentai20.io (WordPress/MangaReader theme)"""
    
    name = "hentai20"
    base_url = "https://hentai20.io"
    
    def search(self, query: str) -> List[Manga]:
        """Search for manga on Hentai20"""
        ...
        for link in soup.select('a[href*="/manga/"]'):
            ...
            if not title or len(title) < 2:
                ...
                if parent:
                    title_elem = parent.select_one('.post-title, .title, h3, h4')
                    if title_elem:
    ...
    def get_chapters(self, manga_url: str) -> List[Chapter]:
        ...
        # Find chapter links (MangaReader theme patterns)
        for link in soup.select('.wp-manga-chapter a, .chapter-item a, li.chapter a, a[href*="/chapter"]'):
            href = link.get('href', '')
            if not href or href in seen:
                continue
            seen.add(href)
            ...
            match = re.search(r'(?:Chapter|Ch\.?|Chap)\s*(\d+\.?\d*)', text, re.I)
            if match:
    ...
    def get_pages(self, chapter_url: str) -> List[str]:
        ...
        # MangaReader theme reader patterns
        # Pattern 1: reader-area class
        reader = soup.select_one('.reader-area, .reading-content')
        if reader:
        ...
        # Pattern 2: page-break divs
        if not pages:
            for img in soup.select('.page-break img, .wp-manga-chapter-img'):
                src = img.get('data-src') or img.get('src')
                if src and src not in pages:
                    if not src.startswith('http'):
                        src = urljoin(self.base_url, src)
        ...
        if not pages:
            for img in soup.select('img'):
                ...
                if any(skip in src.lower() for skip in ['icon', 'logo', 'avatar', 'banner', 'thumb', 'favicon', 'gravatar', 'wp-content/themes', 'ads']):
                    continue
