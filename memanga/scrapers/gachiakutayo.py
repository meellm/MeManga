"""
GachiakutaYo scraper (gachiakutayo.com) - Gachiakuta dedicated site.

Uses Nuxt.js with SSR, images hosted on assets.gachiakutayo.com.
...
class GachiakutaYoScraper(BaseScraper):
    """Scraper for gachiakutayo.com - Gachiakuta manga."""
    ...
    def search(self, query: str) -> List[Manga]:
        ...
        return [Manga(
            title="Gachiakuta",
            url=f"{self.base_url}/",
            cover_url="https://gachiakutayo.com/volume-covers3/gachiakutayo.webp",
        )]
    ...
    def get_pages(self, chapter_url: str) -> List[str]:
        ...
        for img in soup.select("div.images img, img.l"):
            ...
            if "assets.gachiakutayo.com" in url:
                if url not in seen:
                    seen.add(url)
                    pages.append(url)
        ...
        if not pages:
            for meta in soup.select('meta[property="og:image"]'):
                ...
                if url and "assets.gachiakutayo.com" in url:
                    if url not in seen:
                        seen.add(url)
                        pages.append(url)
