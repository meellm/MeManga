"""
WitchWatch scraper - witch-watch.com
Witch Watch by Kenta Shinohara dedicated manga site
...
class WitchWatchScraper(BaseScraper):
    """Scraper for witch-watch.com - Witch Watch dedicated manga site."""
    ...
    def search(self, query: str) -> List[Manga]:
        """Search for manga - returns Witch Watch if query matches."""
        query_lower = query.lower()
        
        # This is a Witch Watch dedicated site
        if any(x in query_lower for x in ['witch watch', 'witchwatch', 'kenta shinohara', 'nico morihira']):
            return [Manga(
                title="Witch Watch",
                url=self.base_url,
                cover_url=f"{self.base_url}/wp-content/uploads/2024/01/witch-watch-manga.webp",
                description="Witch Watch manga by Kenta Shinohara. Morihito Otogi is a teen ogre who's been ordered to protect Nico, a young girl who wants to become a full-fledged witch."
            )]
        return []
    
    def get_chapters(self, manga_url: str) -> List[Chapter]:
        """Get all chapters for Witch Watch."""
        html = self._get_html(self.base_url)
        soup = BeautifulSoup(html, 'html.parser')
        chapters = []
        
        # Find all chapter links
        chapter_links = soup.select('a[href*="witch-watch-chapter"]')
        seen_urls = set()
        ...
        logger.info(f"Found {len(unique_chapters)} chapters for Witch Watch")
