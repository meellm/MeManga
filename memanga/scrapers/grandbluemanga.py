"""
GrandBlueManga scraper (grandbluemanga.com) - Grand Blue Dreaming dedicated site.

Uses Nuxt.js with SSR, images hosted on assets.grandbluemanga.com.
...
class GrandBlueMangaScraper(BaseScraper):
    """Scraper for grandbluemanga.com - Grand Blue Dreaming manga."""
    ...
    def search(self, query: str) -> List[Manga]:
        ...
        return [Manga(
            ...
            url=f"{self.base_url}/",
            cover_url="https://grandbluemanga.com/volume-covers3/grandbluemanga.webp",
        )]
    ...
    def get_pages(self, chapter_url: str) -> List[str]:
        ...
        for img in soup.select("div.images img, img.l"):
            ...
            if "assets.grandbluemanga.com" in url:
                if url not in seen:
                    seen.add(url)
                    pages.append(url)
        ...
        if not pages:
            for meta in soup.select('meta[property="og:image"]'):
                ...
                if url and "assets.grandbluemanga.com" in url:
                    if url not in seen:
                        seen.add(url)
                        pages.append(url)
