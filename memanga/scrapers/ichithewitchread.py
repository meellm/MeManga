"""
IchiTheWitchRead scraper (ichithewitchread.com) - Ichi the Witch dedicated site.

Uses Nuxt.js with SSR, images hosted on assets.ichithewitchread.com.
...
class IchiTheWitchReadScraper(BaseScraper):
    """Scraper for ichithewitchread.com - Ichi the Witch manga."""
    
    name = "ichithewitchread"
    ...
    def search(self, query: str) -> List[Manga]:
        ...
        return [Manga(
            ...
            url=f"{self.base_url}/",
            cover_url="https://ichithewitchread.com/volume-covers3/ichithewitchread.webp",
        )]
    ...
    def get_pages(self, chapter_url: str) -> List[str]:
        ...
        for img in soup.select("div.images img, img.l"):
            ...
            if "assets.ichithewitchread.com" in url:
                if url not in seen:
                    seen.add(url)
                    pages.append(url)
        ...
        if not pages:
            for meta in soup.select('meta[property="og:image"]'):
                ...
                if url and "assets.ichithewitchread.com" in url:
                    if url not in seen:
                        seen.add(url)
                        pages.append(url)
