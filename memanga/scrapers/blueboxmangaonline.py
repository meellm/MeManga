"""
BlueBoxMangaOnline scraper (blue-box-manga.online) - Blue Box / Ao no Hako dedicated site.

Uses WordPress with Comic Easel plugin, images hosted on Blogger CDN with data-lazy-src.
"""
...
class BlueBoxMangaOnlineScraper(BaseScraper):
    """Scraper for blue-box-manga.online - Blue Box (Ao no Hako) manga."""
    
    name = "blueboxmangaonline"
    ...
    def get_pages(self, chapter_url: str) -> List[str]:
        ...
        # Find images - they use data-lazy-src for lazy loading
        for img in entry.find_all("img"):
            # Try data-lazy-src first, then src
            src = img.get("data-lazy-src") or img.get("src") or ""
            src = src.strip()
            
            # Skip placeholder SVGs and data URIs
            if "data:image" in src or not src or src.startswith("data:"):
                continue
