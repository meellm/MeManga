...
Popular manga reader with clean interface.
"""
...
class MangaReaderScraper(BaseScraper):
    """Scraper for MangaReader.to"""
    ...
    def search(self, query: str) -> List[Manga]:
        ...
        results = []
        for item in soup.select(".manga-poster, .item-spc, .mls-wrap .item"):
            link = item.find("a")
            if not link:
                continue
            ...
            # Get title
            title_elem = item.select_one(".manga-name, .manga-title, h3, h2")
    ...
    def get_chapters(self, manga_url: str) -> List[Chapter]:
        ...
        chapters = []
        for link in soup.select(".chapters-list-ul a, .chapter-list a, a[href*='/chapter-']"):
            href = link.get("href", "")
    ...
    def get_pages(self, chapter_url: str) -> List[str]:
        ...
        pages = []
        for img in soup.select(".reading-content img, .container-reader img, .page-img, img[data-src]"):
            src = img.get("data-src") or img.get("src")
            if src and ("chapter" in src.lower() or "manga" in src.lower() or ".jpg" in src.lower() or ".png" in src.lower()):
