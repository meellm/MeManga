"""
Galaxxias scraper (galaxxias.com) - Galaxias manga dedicated site.

Uses Nuxt.js with SSR, images hosted on assets.galaxxias.com.
...
class GalaxxiasScraper(BaseScraper):
    """Scraper for galaxxias.com - Galaxias manga."""
    ...
    def search(self, query: str) -> List[Manga]:
        ...
        return [Manga(
            title="Galaxias",
            url=f"{self.base_url}/",
            cover_url="https://galaxxias.com/volume-covers3/galaxxias.webp",
        )]
    ...
    def get_pages(self, chapter_url: str) -> List[str]:
        ...
        for img in soup.select("div.images img, img.l"):
            ...
            if "assets.galaxxias.com" in url:
                if url not in seen:
                    seen.add(url)
                    pages.append(url)
        ...
        if not pages:
            for meta in soup.select('meta[property="og:image"]'):
                ...
                if url and "assets.galaxxias.com" in url:
                    if url not in seen:
                        seen.add(url)
                        pages.append(url)
