"""
PunpunManga scraper (punpunmanga.com) - Goodnight Punpun (Oyasumi Punpun) dedicated site.

Uses Nuxt.js with SSR, images hosted on assets.punpunmanga.com.
...
class PunpunMangaScraper(BaseScraper):
    """Scraper for punpunmanga.com - Goodnight Punpun manga."""
    ...
    def search(self, query: str) -> List[Manga]:
        ...
        return [Manga(
            ...
            url=f"{self.base_url}/",
            cover_url="https://punpunmanga.com/volume-covers3/punpunmanga.webp",
        )]
    ...
    def get_pages(self, chapter_url: str) -> List[str]:
        ...
        for img in soup.select("div.images img, img.l"):
            ...
            if "assets.punpunmanga.com" in url:
                if url not in seen:
                    seen.add(url)
                    pages.append(url)
        ...
        if not pages:
            for meta in soup.select('meta[property="og:image"]'):
                ...
                if url and "assets.punpunmanga.com" in url:
                    if url not in seen:
                        seen.add(url)
                        pages.append(url)
