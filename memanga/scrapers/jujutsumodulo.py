"""
JujutsuModulo scraper (jujutsumodulo.com) - Jujutsu Kaisen Modulo spin-off dedicated site.

Uses Nuxt.js with SSR, images hosted on assets.jujutsumodulo.com.
...
class JujutsuModuloScraper(BaseScraper):
    """Scraper for jujutsumodulo.com - Jujutsu Kaisen Modulo manga."""
    ...
    def search(self, query: str) -> List[Manga]:
        ...
        return [Manga(
            ...
            url=f"{self.base_url}/",
            cover_url="https://jujutsumodulo.com/volume-covers3/jujutsumodulo.webp",
        )]
    ...
    def get_pages(self, chapter_url: str) -> List[str]:
        ...
        for img in soup.select("div.images img, img.l"):
            ...
            if "assets.jujutsumodulo.com" in url:
                if url not in seen:
                    seen.add(url)
                    pages.append(url)
        ...
        if not pages:
            for meta in soup.select('meta[property="og:image"]'):
                ...
                if url and "assets.jujutsumodulo.com" in url:
                    if url not in seen:
                        seen.add(url)
                        pages.append(url)
