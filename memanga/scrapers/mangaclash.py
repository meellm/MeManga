"""
MangaClash scraper - Manga aggregator
Site: mangaclash.com
...
class MangaClashScraper(PlaywrightScraper):
    """Scraper for MangaClash."""
    ...
    def search(self, query: str) -> List[Manga]:
        ...
        for item in soup.select('.c-tabs-item__content, .search-wrap, .row.c-tabs, .page-item-detail'):
            ...
            # Get title
            title_el = item.select_one('.post-title, h3, h4, .item-title')
    ...
    def get_chapters(self, manga_url: str) -> List[Chapter]:
        ...
        for link in soup.select('.wp-manga-chapter a, li.wp-manga-chapter a, a[href*="chapter"]'):
            href = link.get('href', '')
            if not href or href in seen:
                continue
