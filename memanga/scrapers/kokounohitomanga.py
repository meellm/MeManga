"""
KokouNoHitoManga scraper (kokounohito.com) - The Climber (Kokou no Hito) dedicated site.

Uses Nuxt.js with SSR, images hosted on assets.kokounohito.com.
...
class KokouNoHitoMangaScraper(BaseScraper):
    """Scraper for kokounohito.com - The Climber (Kokou no Hito) manga."""
    
    name = "kokounohitomanga"
    ...
    def search(self, query: str) -> List[Manga]:
        ...
        return [Manga(
            title="The Climber (Kokou no Hito)",
            url=f"{self.base_url}/",
            cover_url="https://kokounohito.com/volume-covers3/kokounohito.webp",
        )]
    ...
    def get_pages(self, chapter_url: str) -> List[str]:
        ...
        for img in soup.select("div.images img, img.l"):
            ...
            if "assets.kokounohito.com" in url:
                if url not in seen:
                    seen.add(url)
                    pages.append(url)
        ...
        if not pages:
            for meta in soup.select('meta[property="og:image"]'):
                ...
                if url and "assets.kokounohito.com" in url:
                    if url not in seen:
                        seen.add(url)
                        pages.append(url)
