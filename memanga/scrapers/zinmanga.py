"""
ZinManga scraper - Manga aggregator
Site: zinmanga.com
...
class ZinMangaScraper(PlaywrightScraper):
    """Scraper for ZinManga."""
    ...
    def search(self, query: str) -> List[Manga]:
        ...
        for item in soup.select('.c-tabs-item__content, .search-wrap, .row.c-tabs'):
    ...
    def get_chapters(self, manga_url: str) -> List[Chapter]:
        ...
        for link in soup.select('.wp-manga-chapter a, .chapter-link, li.wp-manga-chapter a'):
            href = link.get('href', '')
            if not href or href in seen:
                continue
